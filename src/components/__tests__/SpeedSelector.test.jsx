...
describe('SpeedSelector', function SpeedSelectorTests() {
  ...
  it('has the correct min and max bounds', function testSliderBounds() {
    ...
    expect(slider).toHaveAttribute('min', String(getConstant('MIN_SIMULATION_SPEED_MS')));
    expect(slider).toHaveAttribute('max', String(getConstant('MAX_SIMULATION_SPEED_MS')));
  });
  ...
  it('calls onSpeedChange with a number when the slider moves', function testChangeHandlerReceivesNumber() {
    ...
    fireEvent.change(screen.getByLabelText('Geschwindigkeit'), { target: { value: '500' } });
